...
def GUI1(window):
    ...
    def getOnlyOnSegmentPoints(segment_first_extremity, segment_last_extremity, points, maximal_allowed_distance):
        ...
        p = Point((k * (segment_last_point_x - segment_first_point_x) + (segment_first_point_x)),
                  (k * (segment_last_point_y - segment_first_point_y) + (segment_first_point_y)));
            ...
            # to determine direction
                ...
                if not to.counted:
                    ...
                    if getOnlyOnSegmentPoints(pointEntree1, pointEntree2, point, 1.5):
                        ...
                        print("Entree1 x" + str(pointEntree1.x) + " y" + str(pointEntree1.y) + "|| x" + str(
                            pointEntree2.x) + " y" + str(pointEntree2.y))
                    ...
                    # elif direction > 0 and centroid[1] in range(frameHeight // 2 - 30, frameHeight // 2 + 30)
                        ...
                        print("x" + str(centroid[0]) + " y" + str(centroid[1]))
                        print("Entree1 x" + str(pointSortie1.x) + " y" + str(pointSortie1.y) + "|| x" + str(
                            pointSortie2.x) + " y" + str(pointSortie2.y))
                        # if type is "person":
                        #    totalDown += 1
    ...
    def search_video2():
        ...
        if (videos):
            ...
            for videoT in videos:
                ...
                videTmp = Video(videoT, "Non spécifié", str(datetime.timedelta(seconds=durationT)).split(".")[0],
                                "En attente")
                videoList.append(videTmp)
                tree.insert("", 'end', text="L1", values=(
                videoT, "Non spécifié", str(datetime.timedelta(seconds=durationT)).split(".")[0], "En attente"))
            # cap = cv2.VideoCapture(video)
    ...
    def insertData():
        ...
        histo = Historique_comptage(0, "Chemin", "2000-12-02", "1:05:00", "FAKO", 2, 2, 2, 2, 8, 0)
    ...
    def insertData2():
        global historiqueList
        histoService = HistoriqueComtpageService()
        histo = Historique_comptage(0, "Chemin", "2000-12-02", "0:09:00", "TSY FAKO", 2, 4, 3, 2, 11, 0)
        histoService.insertIntoTable(histo)
        # tree.insert("", 'end', text="L1",values=(, "Non spécifié", str(datetime.timedelta(seconds=duration)).split(".")[0], "En attente"))
        refreshHisto()
    ...
    def detect_objectsYolo():
        ...
        while cap.isOpened():
            ...
            if hasFrame is False:
                ...
                comptageHisto = Historique_comptage(0, videoList[indexVideo].chemin, "2000-20-12",
                                                    videoList[indexVideo].duree, videoList[indexVideo].lieu,
                                                    totalUpCar + totalDownCar, totalUpBus + totalDownBus,
                                                    totalUpTruck + totalDownTruck, totalDownMotor + totalUpMotor,
                                                    totalDownCar + totalDownTruck + totalDownBus,
                                                    totalUpCar + totalUpTruck + totalUpBus)
    ...
    def RefreshLabel():
        ...
        label6 = ttk.Label(canvas1, text=totalUpCar)
        label7 = ttk.Label(canvas1, text=totalDownCar)
        label8 = ttk.Label(canvas1, text=totalUpTruck)
        label9 = ttk.Label(canvas1, text=totalDownTruck)
        label10 = ttk.Label(canvas1, text=totalUpBus)
        label11 = ttk.Label(canvas1, text=totalDownBus)
    ...
    def refreshHisto(historiqueList):
        global histoDataList
        histoDataList = []
        ...
        for res in results:
            ...
            historiqueList.insert("", 'end', text="L1", values=(
            res.id, res.nom, res.date, res.duree, res.lieu, res.nbCar, res.nbBus, res.nbTruck, res.nbMotorBike,
            res.entree, res.sortie))
    ...
    def findHisto():
        ...
        global histoDataList
        global entryLieu
        compt = Historique_comptage(0, "", "2021-09-29", "7", entryLieu.get(), 21, 2, 3, 4, 4, 7)
        histoDataList = []
        for item in historiqueList.get_children():
            historiqueList.delete(item)
        histoService = HistoriqueComtpageService()
        results = histoService.findByCriteria(compt)
        for res in results:
            # (self, id, nom, date, duree, lieu, nbCar, nbBus, nbTruck, entree, sortie)
            histoDataList.append(res)
            historiqueList.insert("", 'end', text="L1", values=(
    ...
    def showChart(event):
        ...
        stockSplitExp = [int(histoDataList[indice].nbCar * 100) / total, int(histoDataList[indice].nbBus * 100) / total,
                         int(histoDataList[indice].nbTruck * 100) / total,
                         int(histoDataList[indice].nbMotorBike * 100) / total]
        print(stockSplitExp)
    ...
    var6 = tk.IntVar()
    var7 = tk.IntVar()
    c1 = tk.Checkbutton(canvas1, text='Person', variable=var1, onvalue=1, offvalue=0, command=print_selection, fg="red")

    c2 = tk.Checkbutton(canvas1, text='Car', variable=var2, onvalue=1, offvalue=0, command=print_selection, fg="blue")
    c3 = tk.Checkbutton(canvas1, text='Truck', variable=var3, onvalue=1, offvalue=0, command=print_selection,
                        fg="green")
    c4 = tk.Checkbutton(canvas1, text='Bus', variable=var4, onvalue=1, offvalue=0, command=print_selection, fg="yellow")
    c5 = tk.Checkbutton(canvas1, text='MotorBike', variable=var5, onvalue=1, offvalue=0, command=print_selection,
                        fg="orange")
    s = ttk.Style()
    ...
    c6 = ttk.Checkbutton(canvas1, style='B1.TCheckbutton', text='Ligne Entrée', variable=var6, onvalue=1, offvalue=0,
    ...
    s.map('B2.TCheckbutton',
          foreground=[('disabled', 'grey'),
                      ...
                      ('active', 'red')])
    c7 = ttk.Checkbutton(canvas1, style='B2.TCheckbutton', text='Ligne Sortie', variable=var7, onvalue=1, offvalue=0,
                         command=print_selection)
    ...
    vsb.place(x=10 + 1000 + 2, y=40, height=200 + 20)
    ...
    historiqueList.heading("6", text="Car", command=lambda: treeview_sort_column(historiqueList, "6", False))
    historiqueList.heading("7", text="Bus", command=lambda: treeview_sort_column(historiqueList, "7", False))
    historiqueList.heading("8", text="Truck", command=lambda: treeview_sort_column(historiqueList, "8", False))
    historiqueList.heading("9", text="MotorBike", command=lambda: treeview_sort_column(historiqueList, "9", False))
    ...
    refreshHisto(historiqueList)
    ################################Liste comptage en cours#########################

    ################################################

    myFont = font.Font(family='Helvetica', size=10, weight='bold')
    b1 = ttk.Button(canvas1, text="Lancer", command=threadPlay)

    # b1.place(x=190,y=300)
    b1.place(x=150, y=300)
    # b1['font']=myFont
    b4 = ttk.Button(canvas1, text="Pause", command=threadPause)
    b4.state(["disabled"])
    ...
    entry1_img = PhotoImage(master=window,file="plus.png")
    entry2_img = PhotoImage(master=window,file="right-arrow.png")
    entry3_img = PhotoImage(master=window,file="left-arrow.png")
    entry4_img = PhotoImage(master=window,file="excel.png")
    entry5_img = PhotoImage(master=window,file="csv.png")
    ajoutBtn = ttk.Button(frame1, image=entry1_img, compound="left", text="Ajouter à la file d'attente",
                          command=search_video2)
    ...
    exportExcelBtn = ttk.Button(historiqueFrame, image=entry4_img, compound="left", text="Exporter en Excel",
                                command=ExcelExport)
    exportExcelBtn.place(x=10, y=0)
    exportCSVBtn = ttk.Button(historiqueFrame, image=entry5_img, compound="left", text="Exporter en CSV",
                              command=CSVExport)
    ...
    rechercheBtn = ttk.Button(historiqueFrame, compound="left", text="Rechercher", command=showDate)
    rechercheBtn.place(x=200, y=400)
    ######Profil##########
...
if __name__ == "__main__":
    root = tk.Tk()
    abel = tk.Label(root, text="Hello from %s" % __file__)
