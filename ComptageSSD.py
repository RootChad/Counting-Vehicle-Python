...
def postprocess2(frame, outs):
    ...
    for i in range(outs.shape[2]):
        ...
        if confidence > confThreshold:
            classId = int(outs[0, 0, i, 1])

            #cols = frameWidth
            #rows = frameHeight
            xLeftBottom = int(outs[0, 0, i, 3] * frameWidth)
            yLeftBottom = int(outs[0, 0, i, 4] * frameHeight)
            xRightTop = int(outs[0, 0, i, 5] * frameWidth)
            yRightTop = int(outs[0, 0, i, 6] * frameHeight)

            # Factor for scale to original size of frame
            heightFactor = frame.shape[0] / 300.0
            widthFactor = frame.shape[1] / 300.0
            # Scale object detection to frame
            xLeftBottom = int(  xLeftBottom)
            yLeftBottom = int(yLeftBottom)
            xRightTop = int( xRightTop)
            yRightTop = int( yRightTop)
    ...
    for i in indices:
        ...
        if classIds[i] == 2 and showCar:
            ...
            drawPred(classIds[i], confidences[i],left, top, width, height, frame)
            continue
...
def detect_objectsYolo():
    ...
    while cap.isOpened():
        ...
        if hasFrame is False:
            ...
            break
        frame_resized = cv2.resize(frame, (800, 600), interpolation=cv2.INTER_AREA)
       ...
       # blob = cv2.dnn.blobFromImage(frame_resized,  0.007843, (300, 300), (127.5, 127.5, 127.5), False)
        ...
        postprocess2(frame_resized, outs)
...
def ExcelExport():
    ...
    file = filedialog.asksaveasfile(filetypes=files, defaultextension='.xls')
...
def CSVExport():
    ...
    global histoDataList
    files = [('CSV Files', '*.csv')]
    file = filedialog.asksaveasfile(filetypes=files, defaultextension='.csv')
    #print('file:', file)
...
# create frames
menuFrame = ttk.Frame(notebook, width=1500, height=800)
historiqueFrame = ttk.Frame(notebook, width=1500, height=800)
profilFrame = ttk.Frame(notebook, width=1500, height=800)
